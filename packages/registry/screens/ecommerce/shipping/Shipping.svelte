<script lang="ts">
  // Moderno screen — Shipping (Svelte). Copy-paste; edit freely.
  ...
  // callback — the Payment screen re-surfaces the same block, pre-filled, to
  // let the user review or adjust the card details before moving on.
...
<div class="md-shipping">
  <div class="md-shipping__header">
    ...
    {#if onBack}
      <button type="button" class="md-shipping__link" onclick={() => onBack?.()}>{backLabel}</button>
    {/if}
  </div>
