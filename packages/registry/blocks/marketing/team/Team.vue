...
// Moderno block — Team (Vue). Copy-paste; edit freely.
    ...
    <div class="md-team__grid">
      <Card v-for="member in members" :key="member.name">
        <div class="md-team__member">
          ...
          <p v-if="member.role" class="md-team__member-role">{{ member.role }}</p>
          <p v-if="member.bio" class="md-team__member-bio">{{ member.bio }}</p>
        </div>
      </Card>
    </div>
  </section>
