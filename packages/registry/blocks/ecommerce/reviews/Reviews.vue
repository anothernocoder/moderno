...
// Moderno block — Reviews (Vue). Copy-paste; edit freely.
...
function percentage(count: number): number {
  return props.reviewCount > 0 ? Math.round((count / props.reviewCount) * 100) : 0
}
</script>

<template>
  <section class="md-reviews" :aria-label="heading">
    <h2 v-if="heading" class="md-reviews__heading">{{ heading }}</h2>
    <div class="md-reviews__wrapper">
      <div class="md-reviews__summary">
        <div class="md-reviews__average-block">
          ...
          <span class="md-reviews__star-row" role="img" :aria-label="`${averageRating} de 5 estrellas`">
            <svg
              ...
              width="20"
              height="20"
              :fill="index - 1 < filledStars(averageRating) ? 'currentColor' : 'none'"
              stroke="currentColor"
              stroke-width="1.5"
              aria-hidden="true"
            >
        ...
        <div class="md-reviews__breakdown-list">
          <div v-for="item in breakdown" :key="item.stars" class="md-reviews__breakdown-row">
            ...
            <Progress :model-value="percentage(item.count)" :max="100" :show-value="false" />
            <span class="md-reviews__breakdown-count">{{ item.count }}</span>
          </div>
        </div>
      </div>
      <div class="md-reviews__list">
        <Card v-for="(review, index) in reviews" :key="review.name ?? index">
          <div class="md-reviews__review-header">
            ...
            <div class="md-reviews__reviewer-info">
              <span class="md-reviews__reviewer-name-row">
                ...
                <Badge v-if="review.verified" variant="success">Compra verificada</Badge>
              </span>
              <span class="md-reviews__review-meta">
                <span class="md-reviews__star-row" role="img" :aria-label="`${review.rating} de 5 estrellas`">
                  <svg
                    v-for="index in 5"
                    :key="`r-${review.name}-${index}`"
                    viewBox="0 0 24 24"
                    width="14"
                    height="14"
                    :fill="index - 1 < filledStars(review.rating) ? 'currentColor' : 'none'"
                    stroke="currentColor"
                    stroke-width="1.5"
                    aria-hidden="true"
                  >
