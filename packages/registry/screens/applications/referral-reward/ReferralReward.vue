...
// Moderno screen — ReferralReward (Vue). Copy-paste; edit freely.
...
const emit = defineEmits<{ claim: []; back: [] }>()
</script>
...
<template>
  <div class="md-referral-reward">
    <div class="md-referral-reward__card">
      ...
      <div v-if="invites && invites.length > 0" class="md-referral-reward__list">
        <div
          v-for="(invite, index) in invites"
          :key="invite.id"
          class="md-referral-reward__row"
          :class="{ 'md-referral-reward__row--last': index === invites.length - 1 }"
        >
      ...
      <Button variant="primary" class="md-referral-reward__cta" @click="emit('claim')">{{ claimLabel }}</Button>
      <button type="button" class="md-referral-reward__back" @click="emit('back')">{{ backLabel }}</button>
    </div>
  </div>
</template>
...
.md-referral-reward__cta {
  width: 100%;
}
.md-referral-reward__back {
  background: none;
  border: none;
  padding: 0;
  margin-top: 16px;
