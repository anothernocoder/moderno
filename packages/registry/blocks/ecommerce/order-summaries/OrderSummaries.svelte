<script lang="ts">
  // Moderno block — OrderSummaries (Svelte). Copy-paste; edit freely.
  ...
  let subtotal = $derived(items.reduce((sum, item) => sum + lineTotal(item), 0))
...
<div class="md-order-summaries" aria-label={heading}>
  {#if heading}
    <div class="md-order-summaries__section md-order-summaries__section--heading">
  ...
  <section class="md-order-summaries__section">
    {#if items.length === 0}
      <p class="md-order-summaries__empty">{emptyMessage}</p>
    {:else}
      <div class="md-order-summaries__list">
        {#each items as item (item.id)}
          <div class="md-order-summaries__item-row">
            <div class="md-order-summaries__item-info">
              <p class="md-order-summaries__item-title">{item.title}</p>
              {#if item.quantity && item.quantity > 1}
                <p class="md-order-summaries__item-qty">Cantidad: {item.quantity}</p>
              {/if}
            </div>
