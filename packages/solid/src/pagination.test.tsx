...
describe('Pagination', () => {
  ...
  it('renders an ellipsis with the md-pagination-ellipsis class when the page range is truncated', () => {
    const { container } = render(() => (
      <Pagination count={200} defaultPageSize={10} defaultPage={10} siblingCount={1} boundaryCount={1} />
    ))
  ...
  it('supports a controlled page prop', async () => {
    ...
    render(() => <Pagination count={30} defaultPageSize={10} page={page()} onPageChange={setPage} />)
    expect(item(1)).toHaveAttribute('aria-current', 'page')

    setPage(3)
    await waitFor(() => expect(item(3)).toHaveAttribute('aria-current', 'page'))
  })
})
