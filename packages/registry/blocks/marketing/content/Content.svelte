    ...
    {#each sections as section, index (section.heading ?? index)}
      <div>
        ...
        <div class={index > 0 ? 'md-content__section md-content__section--divided' : 'md-content__section'}>
          {#if section.heading}<h3 class="md-content__section-heading">{section.heading}</h3>{/if}
          {#each section.body as paragraph, paragraphIndex (paragraphIndex)}
            <p class="md-content__body">{paragraph}</p>
          {/each}
        </div>
      </div>
    {/each}
