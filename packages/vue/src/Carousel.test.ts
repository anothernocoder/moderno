...
function renderCarousel(props: Record<string, unknown> = {}, onPageChange?: (page: number) => void) {
  return render({
    render: () =>
      h(Carousel.Root, { slideCount: 3, 'onUpdate:page': onPageChange, ...props }, () => [
        h(Carousel.Control, null, () => [h(Carousel.PrevTrigger), h(Carousel.NextTrigger)]),
        h(Carousel.ItemGroup, null, () => [
          h(Carousel.Item, { index: 0 }, () => 'Slide 1'),
          h(Carousel.Item, { index: 1 }, () => 'Slide 2'),
          h(Carousel.Item, { index: 2 }, () => 'Slide 3'),
        ]),
        h(Carousel.IndicatorGroup, null, {
          default: ({ index }: { index: number }) => h(Carousel.Indicator, { index }),
        }),
      ]),
  })
}
...
describe('Carousel', () => {
  ...
  it('throws when a part renders outside Carousel.Root', () => {
    ...
    expect(() => render(Carousel.ItemGroup)).toThrow('Moderno: <Carousel.ItemGroup> must be used inside <Carousel.Root>')
    spy.mockRestore()
  })
})
