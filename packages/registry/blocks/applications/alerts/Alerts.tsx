...
const itemStyle: CSSProperties = { position: 'relative' }
...
const actionStyle: CSSProperties = { marginLeft: '12px' }

/**
 * Moderno block — Alerts (React). Copy-paste; edit freely.
 ...
 */
export function Alerts({ items = DEFAULT_ITEMS, onDismiss, onAction }: AlertsProps) {
      ...
      {visible.map((item) => (
        <div key={item.id} style={itemStyle}>
          <Alert variant={item.variant} title={item.title}>
            {item.description}
            {item.actionLabel ? (
              <Button variant="secondary" size="sm" style={actionStyle} onClick={() => onAction?.(item.id)}>
                {item.actionLabel}
              </Button>
            ) : null}
          </Alert>
