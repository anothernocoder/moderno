...
// Moderno block — List (Vue). Copy-paste; edit freely.
    ...
    <template v-for="(item, index) in props.items" :key="item.id">
      <div class="md-list__row">
        ...
        <div class="md-list__text">
          ...
          <p v-if="item.subtitle" class="md-list__subtitle">{{ item.subtitle }}</p>
        </div>
        <Badge v-if="item.status" :variant="item.statusVariant" dot>{{ item.status }}</Badge>
        <span v-if="item.meta" class="md-list__meta">{{ item.meta }}</span>
        <Button
          variant="secondary"
          size="sm"
