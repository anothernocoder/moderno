...
// Moderno block — ProductFeatures (Vue). Copy-paste; edit freely.
...
<template>
  <section class="md-product-features" :aria-label="heading">
    <h2 v-if="heading" class="md-product-features__heading">{{ heading }}</h2>
    <div class="md-product-features__grid">
      <Card v-for="(feature, index) in props.features" :key="feature.label ?? index">
        <template #title>
          <span class="md-product-features__title-row">
            ...
            >
              <circle
                v-for="(c, cIndex) in iconSpec(feature.icon).circles"
                :key="`c-${cIndex}`"
                :cx="c.cx"
                :cy="c.cy"
                :r="c.r"
              />
              <path v-for="(d, dIndex) in iconSpec(feature.icon).paths" :key="`p-${dIndex}`" :d="d" />
            </svg>
