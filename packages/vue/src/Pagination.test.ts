...
describe('Pagination', () => {
  it('renders the root nav, prev/next triggers and page items with the md-* class contract', () => {
    render(Pagination, { props: { count: 30, defaultPageSize: 10, defaultPage: 1 } })
  ...
  it('disables next on the last page', () => {
    render(Pagination, { props: { count: 30, defaultPageSize: 10, defaultPage: 3 } })
  ...
  it('renders an ellipsis with the md-pagination-ellipsis class when the page range is truncated', () => {
    const { container } = render(Pagination, {
      props: { count: 200, defaultPageSize: 10, defaultPage: 10, siblingCount: 1, boundaryCount: 1 },
    })
    expect(container.querySelectorAll('.md-pagination-ellipsis').length).toBeGreaterThan(0)
  })

  it('moves to the next page on click and emits update:modelValue with the public API', async () => {
    render(Pagination, { props: { count: 30, defaultPageSize: 10, defaultPage: 1 } })

    await fireEvent.click(screen.getByLabelText('next page'))

    await waitFor(() => expect(item(2)).toHaveAttribute('aria-current', 'page'))
  })

  it('supports a controlled modelValue prop', async () => {
    const { rerender } = render(Pagination, { props: { count: 30, defaultPageSize: 10, modelValue: 1 } })
    expect(item(1)).toHaveAttribute('aria-current', 'page')
